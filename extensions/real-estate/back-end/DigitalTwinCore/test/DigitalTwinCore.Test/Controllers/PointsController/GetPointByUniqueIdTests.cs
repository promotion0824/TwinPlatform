...
{
    public class GetPointByUniqueIdTests : BaseInMemoryTest
    {
        public GetPointByUniqueIdTests(ITestOutputHelper output) : base(output)
        {
        }

        [Fact(Skip = "Fix later")]
        public async Task PointExist_GetPointByUniqueId_ReturnsThatPoint()
        {
            ...
            var response = await client.GetAsync($"sites/{siteId}/points/{expectedPoint.UniqueId}");
        ...
        [Fact(Skip = "Fix later")]
        public async Task PointDoesNotExist_GetPointByUniqueId_ReturnsNotFound()
        {
            var userId = Guid.NewGuid();
            var siteId = Guid.NewGuid();
            ...
            using var client = server.CreateClient(null, userId);
            var response = await client.GetAsync($"sites/{siteId}/points/{Guid.NewGuid()}");
