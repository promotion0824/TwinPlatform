...
{
    public class GetAssetByIdTests : BaseInMemoryTest
    {
        public GetAssetByIdTests(ITestOutputHelper output) : base(output)
        {
        }

        [Fact(Skip = "Fix later")]
        public async Task NotUsingBuildingSpecificModels_GetAssetById_ReturnsAsset()
        {
            ...
            var response = await client.GetAsync($"sites/{siteId}/assets/twinId/{expectedAsset.Id}");
        ...
        [Fact(Skip = "Fix later")]
        public async Task InvalidAssetId_GetAssetById_ReturnsNotFound()
        {
            var userId = Guid.NewGuid();
            var siteId = Guid.NewGuid();
            ...
            var response = await client.GetAsync($"sites/{siteId}/assets/twinId/This-id-does-not-exist");

            response.StatusCode.Should().Be(HttpStatusCode.NotFound);
        }

        [Fact(Skip = "Fix later")]
        public async Task AssetOfDifferentSite_GetAssetById_ReturnsNotFound()
        {
            var userId = Guid.NewGuid();
            var siteId = Guid.NewGuid();
            ...
            var response = await client.GetAsync($"sites/{siteId}/assets/twinId/{assetOfDifferentSite.Id}");
        ...
        [Fact(Skip = "Fix later")]
        public async Task UsingBuildingSpecificModels_GetAssetById_ReturnsAsset()
        {
            var userId = Guid.NewGuid();
            var siteId = Guid.NewGuid();
