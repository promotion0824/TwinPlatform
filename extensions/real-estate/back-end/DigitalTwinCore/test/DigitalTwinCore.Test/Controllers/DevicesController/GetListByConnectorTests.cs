...
namespace DigitalTwinCore.Test.Controllers.DevicesController
{
    public class GetListByConnectorTests : BaseInMemoryTest
    {
        ...
        public async Task DeviceExists_GetList_ReturnsDevice()
        {
            ...
            var otherDevice = AdtSetupHelper.CreateTwin("Controller", "Test Device2", siteId);
            otherDevice.CustomProperties.Add(Properties.ConnectorID, Guid.NewGuid().ToString());
            var otherDeviceId = setup.AddTwin(otherDevice);
            setup.AddRelationship(otherDeviceId, AdtSetupHelper.MakeId(siteId, "Level1"), Relationships.LocatedIn);
            ...
            var response = await client.GetAsync($"sites/{siteId}/connectors/{connectorId}/devices");
        ...
        public async Task DeviceExists_GetList_WithIncludePoints_ReturnsDeviceWithPoints()
        {
            ...
            var response = await client.GetAsync($"sites/{siteId}/connectors/{connectorId}/devices?includePoints=true");
        ...
        public async Task InvalidSiteId_GetList_ReturnsNotFound()
        {
            ...
            var response = await client.GetAsync($"sites/{Guid.NewGuid()}/connectors/{Guid.NewGuid()}/devices");

            response.StatusCode.Should().Be(HttpStatusCode.NotFound);
        }

        [Fact(Skip = "Fix later")]
        public async Task DeviceExistsForDifferentConnector_GetList_ReturnsEmptyList()
        {
            var userId = Guid.NewGuid();
            var siteId = Guid.NewGuid();
            var connectorId = Guid.NewGuid();
            ...
            var expectedDevice = AdtSetupHelper.CreateTwin("Controller", "Test Device", siteId);
            expectedDevice.CustomProperties.Add(Properties.ConnectorID, Guid.NewGuid().ToString());
            var expectedDeviceId = setup.AddTwin(expectedDevice);
            setup.AddRelationship(expectedDeviceId, AdtSetupHelper.MakeId(siteId, "Level1"), Relationships.LocatedIn);
