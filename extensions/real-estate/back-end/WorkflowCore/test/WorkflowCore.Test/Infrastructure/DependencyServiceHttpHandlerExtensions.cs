...
namespace Willow.Tests.Infrastructure
{
    public static class MockHttpMessageHandlerExtensions
    {
        ...
        {
            return setup.ReturnsResponse(JsonSerializer.Serialize(content, JsonSerializerExtensions.DefaultOptions), "application/json");
        }
        ...
        {
            return setup.ReturnsResponse(statusCode, JsonSerializer.Serialize(content, JsonSerializerExtensions.DefaultOptions), "application/json");
        }
        ...
        public static ISetup<HttpMessageHandler, Task<HttpResponseMessage>> SetupRequestWithExpectedBody<TRequestJsonType>(this DependencyServiceHttpHandler handler, HttpMethod method, string requestUrl, TRequestJsonType expectedRequestBody)
        {
            return handler.SetupRequest(method, requestUrl, async(message) =>
            {
                ...
                var requestBody = JsonSerializer.Deserialize<TRequestJsonType>(stringContent, JsonSerializerExtensions.DefaultOptions);
