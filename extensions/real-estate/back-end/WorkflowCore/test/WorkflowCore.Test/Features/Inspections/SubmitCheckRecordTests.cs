...
namespace WorkflowCore.Test.Features.Inspections
{
    public class SubmitCheckRecordTests : BaseInMemoryTest
    {
        ...
        public async Task CheckRecordExists_SubmitCheckRecord_CheckRecordIsUpdated(
            CheckType checkType,
            ...
            double? expectedNumberValue, string expectedStringValue)
        {
            ...
            using (var client = server.CreateClient(null))
            {
                ...
                var response = await client.PutAsJsonAsync(
                    $"sites/{inspectionEntity.SiteId}/inspections/{inspectionEntity.Id}/lastRecord/checkRecords/{checkRecordEntity.Id}",
                    request);
                ...
                db.CheckRecords.First().Attachments.Should().Be(JsonSerializer.Serialize(attachments, JsonSerializerExtensions.DefaultOptions));
        ...
        public async Task InputValueWillTriggerInsightGeneration_SubmitCheckRecord_InsightInformationIsReturned(
        ...
        public async Task InputValueWillTriggerInsightGenerationButCheckCannotGenerateInsight_SubmitCheckRecord_NoInsightInformationReturned(
            CheckType checkType, double? checkMinValue, double? checkMaxValue,
            double? inputtedNumberValue, string inputtedNotes)
        {
            var userId = Guid.NewGuid();
        ...
        public async Task CheckRecordExists_SubmitCheckRecord_DependentRecordStatusIsUpdated(
            CheckType checkType,
        ...
        [InlineData( null, null, 1.0,2.0)]  //decrease enteredValue
        [InlineData(2.0, 5.0, 66.0, 65.0)]// increase less than minimum percent
        [InlineData(2.0, 5.0, 68.9,65.0)] // increase more than maximum percent
        public async Task InputValueWillTriggerInsightGenerationForTotalType_SubmitCheckRecord_InsightInformationIsReturned(double? checkMinValue, double? checkMaxValue,
    double inputtedNumberValue, double prvNumberValue)
        {
            var userId = Guid.NewGuid();
            var attachments = Fixture.CreateMany<AttachmentBase>(5).ToList();
            var timeZoneId = "AUS Eastern Standard Time";
            ...
            var checkEntity = Fixture.Build<CheckEntity>()
                                     ...
                                     .Without(x => x.LastRecord)
                                     .Create();
            var prvCheckRecordEntity = Fixture.Build<CheckRecordEntity>()
                                           .With(x => x.InspectionId, inspectionEntity.Id)
                                           .With(x => x.CheckId, checkEntity.Id)
                                           .With(x => x.Attachments, "[]")
                                           .With(x => x.NumberValue,prvNumberValue)
                                           .Without(x => x.StringValue)
                                           .Create();
            checkEntity.LastRecordId = prvCheckRecordEntity.Id;
            var checkRecordEntity = Fixture.Build<CheckRecordEntity>()
                .With(x => x.InspectionId, inspectionEntity.Id)
                .With(x => x.CheckId, checkEntity.Id)
                .With(x => x.InspectionRecordId, inspectionEntity.LastRecordId.Value)
            ...
            using (var client = server.CreateClient(null))
            {
                ...
                db.Inspections.Add(inspectionEntity);
                db.Checks.Add(checkEntity);
                db.CheckRecords.Add(prvCheckRecordEntity);
                db.CheckRecords.Add(checkRecordEntity);
                db.SaveChanges();
                ...
                var result = await response.Content.ReadAsAsync<SubmitCheckRecordResponse>();
                    ...
                    result.RequiredInsight.TwinId.Should().Be(inspectionEntity.TwinId);
                    result.RequiredInsight.Priority.Should().Be(3);
                    result.RequiredInsight.Type.Should().Be(SubmitCheckRecordResponse.InsightType.Alert);
