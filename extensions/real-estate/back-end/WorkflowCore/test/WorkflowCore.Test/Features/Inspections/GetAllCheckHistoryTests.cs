    ...
    public class GetAllCheckHistoryTests : BaseInMemoryTest
    {
        ...
        public async Task CheckWithCheckRecordsExist_GetCheckHistory_ReturnThoseCheckRecords()
        {
            ...
            using (var client = server.CreateClient(null))
            {
                ...
                var response = await client.GetAsync($"inspections/{inspection.Id}/checks/history?siteId={siteId}&checkId={checkId}&customerId={site.CustomerId}");
                
                response.StatusCode.Should().Be(HttpStatusCode.OK);
                var result = await response.Content.ReadAsAsync<List<CheckRecordReportDto>>();
                var expectedCheckRecordsDto = CheckRecordReportDto.MapFromModels(CheckRecordEntity.MapToModels(checkRecordEntities.Where(x => x.CheckId == checkId)),
                                                                            imagePathHelper,InspectionEntity.MapToModel(inspection),expectedTwin.Name, site.CustomerId, siteId);
