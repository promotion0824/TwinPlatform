ALTER PROCEDURE [dbo].[GetTicketSequenceNumber]
...
DECLARE @existingTicketSequencePrefix nvarchar(64)
SET @existingTicketSequencePrefix = CONCAT(@sitePrefix,'-',@siteSuffix,'-')
  ...
  BEGIN 
	...
	BEGIN CATCH
		...
		BEGIN TRY
		  ...
		  /* Retrieve the most recent sequence number from the current tickets for the site.  */
			SELECT @startNumber = (SELECT TOP 1 ExtractedNumbers
                                    FROM (
                                         SELECT SequenceNumber,
                                         CAST (
                                               SUBSTRING(
                                                    SequenceNumber,
                                                    LEN(@existingTicketSequencePrefix) + 1,
                                                    LEN(SequenceNumber) - LEN(@existingTicketSequencePrefix)) 
                                         AS INT) 
                                   AS ExtractedNumbers
                                  FROM WF_Ticket where SequenceNumber like @existingTicketSequencePrefix+'%' 
                                ) AS ExtractedNumbers
                                ORDER BY ExtractedNumbers DESC);
