...
namespace PlatformPortalXL.Test.Features.Workflow.Tickets
{
    public class GetTicketCategoriesTests : BaseInMemoryTest
    {
        ...
        [Fact]
        public async Task UserDoesNotHaveCorrectPermission_GetTicketCategories_ReturnsForbidden()
        {
        ...
        [Fact]
        public async Task ThereAreTicketCategories_GetTicketCategories_ReturnsThoseTicketCategories()
        {
            ...
            var expectedTicketCategories = Fixture.Build<TicketCategory>().CreateMany(10).ToList();
            ...
            using (var client = server.CreateClient(null, userId))
            {
                ...
                server.Arrange().GetWorkflowApi()
                    .SetupRequest(HttpMethod.Get, $"sites/{siteId}/tickets/categories")
                    .ReturnsJson(expectedTicketCategories);

                var response = await client.GetAsync($"sites/{siteId}/tickets/categories");

                response.StatusCode.Should().Be(HttpStatusCode.OK);
                var result = await response.Content.ReadAsAsync<List<TicketCategoryDto>>();
                result.Should().BeEquivalentTo(TicketCategoryDto.MapFrom(expectedTicketCategories));
            }
        }

        [Fact]
        public async Task GetTicketCategories_ByScopeId_ReturnsThoseTicketCategories()
        {
            var siteId = Guid.NewGuid();
            ...
            using (var client = server.CreateClientWithPermissionOnSite(userId, Permissions.ViewSites, siteId))
            {
                ...
                server.Arrange().GetWorkflowApi()
                    .SetupRequest(HttpMethod.Get, $"sites/{expectedTwinDto[0].SiteId}/tickets/categories")
                    .ReturnsJson(expectedTicketCategories);

                var response = await client.GetAsync($"sites/{siteId}/tickets/categories?scopeId={scopeId}");
        ...
        [Fact]
        public async Task GetTicketCategories_ByScopeId_UserHasNoAccess_ReturnsForbidden()
        {
            var siteId = Guid.NewGuid();
            var scopeId = Guid.NewGuid().ToString();
            var userId = Guid.NewGuid();
        ...
        [Fact]
        public async Task GetTicketCategories_ByInvalidScopeId_ReturnsForbidden()
        {
            var siteId = Guid.NewGuid();
            var scopeId = Guid.NewGuid().ToString();
            var userId = Guid.NewGuid();
