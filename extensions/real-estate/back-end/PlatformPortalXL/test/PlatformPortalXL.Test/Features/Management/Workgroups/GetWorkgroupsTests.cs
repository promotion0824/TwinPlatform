...
namespace PlatformPortalXL.Test.Features.Management.Workgroups
{
    public class GetWorkgroupsTests : BaseInMemoryTest
    {
        ...
        [Fact]
        public async Task UserDoesNotHaveCorrectPermission_GetWorkgroups_ReturnsForbidden()
        {
            var siteId = Guid.NewGuid();
            using (var server = CreateServerFixture(ServerFixtureConfigurations.Default))
            using (var client = server.CreateClientWithDeniedPermissionOnSite(null, Permissions.ViewUsers, siteId))
            {
                var response = await client.GetAsync($"management/sites/{siteId}/workgroups");
        ...
        [Fact]
        public async Task WorkgroupsExist_GetWorkgroups_ReturnsWorkgroupsList()
        {
            var siteId = Guid.NewGuid();
            ...
            using (var client = server.CreateClientWithPermissionOnSite(null, Permissions.ViewUsers, siteId))
            {
                ...
                result.Should().BeEquivalentTo(WorkgroupDetailDto.MapFromModels(expectedWorkgroups));
            }
        }
    }
}
