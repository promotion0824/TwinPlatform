    ...
    public class GetDigitalTwinCoreAssetCategoryTreeTests : BaseInMemoryTest
    {
        ...
        public async Task WhenAssetExists_GetAssetCategoryTreeByFloorId_ReturnsSpecifiedFloorAssetCategoryTreeOnly()
        {
            ...
            using (var client = server.CreateClientWithPermissionOnSite(null, Permissions.ViewSites, siteId))
            {
                ...
                server.Arrange().GetDigitalTwinApi()
                    .SetupRequest(HttpMethod.Get, $"sites/{siteId}/assets/AssetTree?floorId={floorIds[1]}&isCategoryOnly=False" + string.Join("", AdtConstants.DefaultAdtModels.Select(m => $"&modelNames={m}")))
                    .ReturnsJson(expectedAssetTree);
