    ...
    public class ServerFixture : IDisposable
    {
        ...
        static ServerFixture()
        {
            AssertionOptions.AssertEquivalencyUsing(options =>
                options.Using<DateTime>(ctx => ctx.Subject.Should().BeCloseTo(ctx.Expectation, TimeSpan.FromMinutes(1)))
                    .WhenTypeIs<DateTime>()
            );
        }
            ...
            string username = "test@test.com")
        {
            ...
            client.DefaultRequestHeaders.Authorization =
                new AuthenticationHeaderValue(TestAuthToken.TestScheme, JsonSerializerHelper.Serialize(token));
            return client;
        }
