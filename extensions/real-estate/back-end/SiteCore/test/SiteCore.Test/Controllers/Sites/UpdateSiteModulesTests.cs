using System;
using System.Collections.Generic;
using System.Linq;
using System.Net;
using System.Net.Http;
using System.Net.Http.Json;
using System.Threading.Tasks;
using AutoFixture;
using FluentAssertions;
using Microsoft.EntityFrameworkCore;
using SiteCore.Dto;
using SiteCore.Entities;
using SiteCore.Requests;
using SiteCore.Tests;
using Willow.Tests.Infrastructure;
using Xunit;
using Xunit.Abstractions;

namespace SiteCore.Test.Controllers.Sites
{
    public class UpdateSiteModulesTests : BaseInMemoryTest
    {
        public const string PngBase64 =
            "";

        public UpdateSiteModulesTests(ITestOutputHelper output) : base(output)
        {
        }

        [Fact]
        public async Task NewUpload_Upload3dmodule_ReturnsNoContent()
        {
            var siteId = Guid.NewGuid();
            var floorCode = "_SITE"; ;
            var customerId = Guid.NewGuid();
            var moduleTypeId = Guid.NewGuid();

            var site = Fixture.Build<SiteEntity>()
                .Without(x => x.Floors)
                .Without(x => x.PortfolioId)
                .With(x => x.TimezoneId, "AUS Eastern Standard Time")
                .With(x => x.Postcode, "111250")
                .With(x => x.LogoId, (Guid?)null)
                .With(x => x.Id, siteId)
                .With(x => x.CustomerId, customerId)
                .Create();

            var imageType = Fixture.Build<ModuleTypeEntity>()
                .With(x => x.Id, moduleTypeId)
                .With(x => x.CanBeDeleted, true)
                .With(x => x.Is3D, true)
                .With(x => x.SiteId, siteId)
                .With(x => x.Prefix, "random_")
                .Without(x => x.Modules)
                .Create();

            var moduleRequest = new CreateUpdateModule3DRequest
            {
                Modules3D = new List<Module3DInfo>
                {
                    new Module3DInfo
                    {
                        ModuleName = "site_module.3d",
                        Url = "https://floor.module.3d"
                    }
                }
            };

            using (var server = CreateServerFixture(ServerFixtureConfigurations.InMemoryDb))
            using (var client = server.CreateClient(null))
            {
                var arrangement = server.Arrange();
                var db = arrangement.CreateDbContext<SiteDbContext>();
                db.Sites.Add(site);
                db.ModuleTypes.Add(imageType);
                db.SaveChanges();

                var response = await client.PostAsJsonAsync($"sites/{siteId}/module", moduleRequest);

                response.StatusCode.Should().Be(HttpStatusCode.NoContent);

                var floor = await db.Floors.FirstOrDefaultAsync(x => x.Code == floorCode);
                floor.Should().NotBeNull();
                floor.IsDecomissioned.Should().Be(true);

                var module = await db.Modules.FirstOrDefaultAsync(di => di.Floor.Code == floorCode && di.Url == moduleRequest.Modules3D.First().Url);
                module.Should().NotBeNull();
            }
        }

        [Fact]
        public async Task ReUpload_Upload3dmodule_ReturnsNoContent()
        {
            var siteId = Guid.NewGuid();
            var floorCode = "_SITE";
            var customerId = Guid.NewGuid();
            var moduleTypeId = Guid.NewGuid();

            var site = Fixture.Build<SiteEntity>()
                .Without(x => x.Floors)
                .Without(x => x.PortfolioId)
                .With(x => x.TimezoneId, "AUS Eastern Standard Time")
                .With(x => x.Postcode, "111250")
                .With(x => x.LogoId, (Guid?)null)
                .With(x => x.Id, siteId)
                .With(x => x.CustomerId, customerId)
                .Create();

            var imageType = Fixture.Build<ModuleTypeEntity>()
                .With(x => x.Id, moduleTypeId)
                .With(x => x.CanBeDeleted, true)
                .With(x => x.Is3D, true)
                .With(x => x.SiteId, siteId)
                .With(x => x.Prefix, "random_")
                .Without(x => x.Modules)
                .Create();

            var moduleRequest = new CreateUpdateModule3DRequest
            {
                Modules3D = new List<Module3DInfo>
                {
                    new Module3DInfo
                    {
                        ModuleName = "site_module.3d",
                        Url = "https://floor.module.3d"
                    }
                }
            };

            using (var server = CreateServerFixture(ServerFixtureConfigurations.InMemoryDb))
            using (var client = server.CreateClient(null))
            {
                var arrangement = server.Arrange();
                var db = arrangement.CreateDbContext<SiteDbContext>();
                db.Sites.Add(site);
                db.ModuleTypes.Add(imageType);
                db.SaveChanges();

                var response = await client.PostAsJsonAsync($"sites/{siteId}/module", moduleRequest);
                response.StatusCode.Should().Be(HttpStatusCode.NoContent);

                var response2 = await client.PostAsJsonAsync($"sites/{siteId}/module", moduleRequest);
                response2.StatusCode.Should().Be(HttpStatusCode.NoContent);

                var modules = await db.Modules.Where(di => di.Floor.Code == floorCode).ToListAsync();
                modules.Count.Should().Be(1);
                modules.FirstOrDefault().Url.Should().Be(moduleRequest.Modules3D.First().Url);
            }
        }

        [Fact]
        public async Task ModuleExists_DeleteModule_ReturnsNoContent()
        {
            var siteId = Guid.NewGuid();

            var customerId = Guid.NewGuid();
            var visualId = Guid.NewGuid();
            var moduleId = Guid.NewGuid();
            var moduleTypeId = Guid.NewGuid();

            var site = Fixture.Build<SiteEntity>()
                .Without(x => x.Floors)
                .Without(x => x.PortfolioId)
                .With(x => x.TimezoneId, "AUS Eastern Standard Time")
                .With(x => x.Postcode, "111250")
                .With(x => x.LogoId, (Guid?)null)
                .With(x => x.Id, siteId)
                .With(x => x.CustomerId, customerId)
                .Create();

            var imageType = Fixture.Build<ModuleTypeEntity>()
                .With(x => x.Id, moduleTypeId)
                .With(x => x.CanBeDeleted, true)
                .With(x => x.Is3D, true)
                .With(x => x.SiteId, siteId)
                .With(x => x.Prefix, "random_")
                .Without(x => x.Modules)
                .Create();

            var moduleRequest = new CreateUpdateModule3DRequest
            {
                Modules3D = new List<Module3DInfo>
                {
                    new Module3DInfo
                    {
                        ModuleName = "site_module.3d",
                        Url = "https://floor.module.3d"
                    }
                }
            };

            using (var server = CreateServerFixture(ServerFixtureConfigurations.InMemoryDb))
            using (var client = server.CreateClient(null))
            {
                var arrangement = server.Arrange();

                var db = arrangement.CreateDbContext<SiteDbContext>();
                db.Sites.Add(site);
                db.ModuleTypes.Add(imageType);
                db.SaveChanges();

                await client.PostAsJsonAsync($"sites/{siteId}/3dmodules", moduleRequest);

                var response = await client.DeleteAsync($"sites/{siteId}/module");
                response.StatusCode.Should().Be(HttpStatusCode.NoContent);

                var dbImage = await db.Modules.FirstOrDefaultAsync(di => di.Id == moduleId);
                dbImage.Should().BeNull();
            }
        }

        [Fact]
        public async Task ModuleExists_GetModule_ReturnsModule()
        {
            var siteId = Guid.NewGuid();

            var customerId = Guid.NewGuid();
            var visualId = Guid.NewGuid();
            var moduleId = Guid.NewGuid();
            var moduleTypeId = Guid.NewGuid();

            var site = Fixture.Build<SiteEntity>()
                .Without(x => x.Floors)
                .Without(x => x.PortfolioId)
                .With(x => x.TimezoneId, "AUS Eastern Standard Time")
                .With(x => x.Postcode, "111250")
                .With(x => x.LogoId, (Guid?)null)
                .With(x => x.Id, siteId)
                .With(x => x.CustomerId, customerId)
                .Create();

            var imageType = Fixture.Build<ModuleTypeEntity>()
                .With(x => x.Id, moduleTypeId)
                .With(x => x.CanBeDeleted, true)
                .With(x => x.Is3D, true)
                .With(x => x.SiteId, siteId)
                .With(x => x.Prefix, "random_")
                .Without(x => x.Modules)
                .Create();

            var moduleRequest = new CreateUpdateModule3DRequest
            {
                Modules3D = new List<Module3DInfo>
                {
                    new Module3DInfo
                    {
                        ModuleName = "site_module.3d",
                        Url = "https://floor.module.3d"
                    }
                }
            };

            using (var server = CreateServerFixture(ServerFixtureConfigurations.InMemoryDb))
            using (var client = server.CreateClient(null))
            {
                var arrangement = server.Arrange();

                var db = arrangement.CreateDbContext<SiteDbContext>();
                db.Sites.Add(site);
                db.ModuleTypes.Add(imageType);
                db.SaveChanges();

                await client.PostAsJsonAsync($"sites/{siteId}/module", moduleRequest);

                var response = await client.GetAsync($"sites/{siteId}/module");

                response.StatusCode.Should().Be(HttpStatusCode.OK);
                var result = await response.Content.ReadAsAsync<LayerGroupModuleDto>();

                result.Name.Should().Be(moduleRequest.Modules3D.FirstOrDefault()?.ModuleName);
                result.Path.Should().Be(moduleRequest.Modules3D.FirstOrDefault()?.Url);
                result.ModuleTypeId.Should().Be(moduleTypeId);
            }
        }

        [Fact]
        public async Task ModuleDoesNotExist_GetModule_ReturnsModule()
        {
            var siteId = Guid.NewGuid();
            var customerId = Guid.NewGuid();

            var site = Fixture.Build<SiteEntity>()
                .Without(x => x.Floors)
                .Without(x => x.PortfolioId)
                .With(x => x.TimezoneId, "AUS Eastern Standard Time")
                .With(x => x.Postcode, "111250")
                .With(x => x.LogoId, (Guid?)null)
                .With(x => x.Id, siteId)
                .With(x => x.CustomerId, customerId)
                .Create();

            using (var server = CreateServerFixture(ServerFixtureConfigurations.InMemoryDb))
            using (var client = server.CreateClient(null))
            {
                var arrangement = server.Arrange();

                var db = arrangement.CreateDbContext<SiteDbContext>();
                db.Sites.Add(site);
                db.SaveChanges();

                var response = await client.GetAsync($"sites/{siteId}/module");

                response.StatusCode.Should().Be(HttpStatusCode.NoContent);
            }
        }
    }
}