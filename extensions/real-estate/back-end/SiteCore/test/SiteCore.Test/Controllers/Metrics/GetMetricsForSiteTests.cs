...
{
    public class GetMetricsForSiteTests : BaseInMemoryTest
    {
        public GetMetricsForSiteTests(ITestOutputHelper output) : base(output)
        {
        }
        ...
        public async Task MetricsExist_ReturnsThem()
        {
            ...
            using (var client = server.CreateClient(null))
            {
                ...
                var response = await client.GetAsync($"sites/{siteId}/metrics?start={DateTime.UtcNow.AddDays(-1):yyyy-MM-ddTHH:mm:ssZ}&end={DateTime.UtcNow:yyyy-MM-ddTHH:mm:ssZ}");
