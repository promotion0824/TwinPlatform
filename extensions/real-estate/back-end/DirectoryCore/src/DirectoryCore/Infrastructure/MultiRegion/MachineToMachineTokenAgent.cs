            ...
            var token = _cache.GetOrCreate(
                ...
                (cacheEntry) =>
                {
                    _logger.LogInformation(
                        "Acquiring access token for region {RegionId}",
                        regionId
                    );
                    ...
                    if (region == null)
                    {
                        throw new InvalidOperationException(
                            $"Cannot find the region {regionId} settings."
                        );
                    }
                    var tokenResponse = GetTokenInternal(region).Result;
                    const int cacheSafetyMargin = 60 * 10;
                    cacheEntry.AbsoluteExpirationRelativeToNow = TimeSpan.FromSeconds(
                        tokenResponse.ExpiresIn - cacheSafetyMargin
                    );
