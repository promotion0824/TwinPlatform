...
namespace Microsoft.Extensions.DependencyInjection
{
    public static class ServiceCollectionExtensions
    {
            ...
            if (configuration.GetValue<bool>("EnableSwagger"))
            {
                services.AddSwaggerGen(options =>
                {
                    ...
                    if (configuration.GetValue<string>("Auth0:ClientId") != null)
                    {
                        ...
                        var authServerUrl =
                            $"https://{configuration.GetValue<string>("Auth0:Domain")}";
        ...
        )
        {
            ...
            {
                string token = FetchMachineToMachineToken(
                    apiName,
                    serviceProvider,
        ...
        private static string GetTokenFromAuthenticationHeaderOrCookie(
            IServiceProvider serviceProvider
        )
        {
            var authValue = serviceProvider
            ...
            else
            {
                return serviceProvider
                    ...
                    ?.HttpContext
                    ?.User
                    ?.FindFirst(ClaimTypes.Authentication)
                    ?.Value;
            }
        }

        private static async Task<string> FetchMachineToMachineToken(
            string apiName,
                    ...
                    using (var client = httpClientFactory.CreateClient())
                    {
                        ...
                        if (!response.IsSuccessStatusCode)
                        {
                            ...
                            catch (Exception ex)
                            {
                                logger.LogError(
                                    ex,
                                    "Failed to get access token. Http status code: {StatusCode} Failed to get ResponseBody. {ExceptionMessage}",
                                    response.StatusCode,
                                    ex.Message
                                );
                                throw;
                        ...
                        var tokenResponse =
                            await response.Content.ReadAsAsync<MachineToMachineTokenAgent.TokenResponse>();
        ...
        )
        {
            ...
            if (!authenticationSection.GetChildren().Any())
            {
                throw new InvalidOperationException(
                    $"Missing 'Authentication' for HttpClient {apiName}"
                );
            }
                ...
                services.AddHttpClient(
                    ...
                    (sv, client) =>
                    {
                        ...
                        client.BaseAddress = new Uri(
                            MultiRegionHelper.MultiRegionUrl(baseAddress, regionId)
                        );
        ...
        {
            var managerService = services.FirstOrDefault(
                s => s.ServiceType == typeof(ApplicationPartManager)
            );
            if (managerService != null)
            {
                var applicationParts = (
                    (ApplicationPartManager)managerService.ImplementationInstance
                ).ApplicationParts;
                var exists = applicationParts.Any(
                    p => (p is AssemblyPart) && ((AssemblyPart)p).Assembly == assembly
                );
                if (!exists)
                {
        ...
        private static void SetServiceKeyHeader(
            string serviceKey,
            HttpHeaders headers,
        ...
        )
        {
            if (
                ...
                && clientConfiguration.GetValue<bool>("RequiresServiceKey")
            )
            {
