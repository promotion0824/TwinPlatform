...
namespace System
{
    public static class ServiceProviderExtensions
    {
        public static object CreateInstance(
            ...
            Type type,
            Dictionary<Type, object> additionalObjects
        )
        {
            var allConstructorInfos = type.GetConstructors(
                BindingFlags.Public | BindingFlags.Instance
            );
            var orderedConstructorInfos = allConstructorInfos
                .OrderByDescending(ci => ci.GetParameters().Length)
                .ToList();
            if (orderedConstructorInfos.Count < 1)
            {
                throw new ArgumentException(
                    $"There is no public constructor in type '{type.FullName}'.",
                    nameof(type)
                );
            }
        ...
        )
        {
            ...
            for (var index = 0; index < parameterInfos.Length; index++)
            {
                ...
                if (
                    ...
                    && additionalObjects.ContainsKey(parameterInfo.ParameterType)
                )
                {
        ...
        )
        {
            ...
            var selectedMethods = allMethodInfos
                .Where(method => method.Name.Equals(methodName, StringComparison.InvariantCulture))
                .ToList();
            if (selectedMethods.Count != 1)
            {
                throw new ArgumentException(
                    $"There is more than one '{methodName}' method in type '{instanceType.FullName}'.",
                    nameof(instance)
                );
            }
        ...
        public static TReturn InvokeMethod<TInstance, TReturn>(
            this IServiceProvider serviceProvider,
            TInstance instance,
            string methodName,
            Dictionary<Type, object> additionalObjects
        ...
        public static void InvokeMethod<TInstance>(
            this IServiceProvider serviceProvider,
            TInstance instance,
            string methodName
        )
