...
namespace DirectoryCore.Database.DbUp
{
    /// <summary>
    ...
    // ReSharper restore CheckNamespace
    {
        ...
        /// A builder for a database upgrader designed for SQL Server databases.
        /// </returns>
        private static UpgradeEngineBuilder AzureSqlDatabase(
        ...
        )
        {
            ...
            builder.Configure(
                c =>
                    c.ScriptExecutor = new SqlScriptExecutor(
                        () => c.ConnectionManager,
                        () => c.Log,
                        schema,
                        () => c.VariablesEnabled,
        ...
        )
        {
            ...
            GetMasterConnectionStringBuilder(
                connectionString,
                logger,
                out masterConnectionString,
                out databaseName
            );
            ...
            using (var connection = new SqlConnection(masterConnectionString))
            {
                if (connection.UseManagedIdentity())
                {
                    logger.WriteInformation(
                        "Use ManagedIdentity for the connection: {0}",
                ...
                string collationString = string.IsNullOrEmpty(collation)
                    ? ""
                    : string.Format(CultureInfo.InvariantCulture, @" COLLATE {0}", collation);
        ...
        private static void GetMasterConnectionStringBuilder(
            string connectionString,
            IUpgradeLog logger,
        ...
        )
        {
            ...
            if (string.IsNullOrEmpty(databaseName) || databaseName.Trim() == string.Empty)
            {
                throw new InvalidOperationException(
                    "The connection string does not specify a database name."
                );
            }

            masterConnectionStringBuilder.InitialCatalog = "master";
            var logMasterConnectionStringBuilder = new SqlConnectionStringBuilder(
                masterConnectionStringBuilder.ConnectionString
            )
            {
            ...
            logger.WriteInformation(
                "Master ConnectionString => {0}",
                logMasterConnectionStringBuilder.ConnectionString
            );
