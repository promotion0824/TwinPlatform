...
{
    public class AuthorizeUserPermissionOnPortfolioTests : BaseInMemoryTest
    {
        public AuthorizeUserPermissionOnPortfolioTests(ITestOutputHelper output)
            : base(output)
        {
            Fixture
        ...
        [Fact]
        public async Task AuthorizePortfolioResourcePermission_PermissionNotExists_ReturnNotAuthorized()
        {
            ...
            using (var client = server.CreateClient(null, user.Id, user.Auth0UserId))
            {
                ...
                var response = await client.GetAsync(
                    $"users/{user.Id}/permissions/{permission.Id}/eligibility?portfolioId={portfolio.Id}"
                );
        ...
        [Fact]
        public async Task AuthorizePortfolioResourcePermission_PortfolioPermissionExists_ReturnAuthorized()
        {
            var user = new UserDto { Id = Guid.NewGuid(), Auth0UserId = "auth0 user id" };
        ...
        [Fact]
        public async Task AuthorizePortfolioResourcePermission_CustomerPermissionExists_ReturnAuthorized()
        {
            var user = new UserDto { Id = Guid.NewGuid(), Auth0UserId = "auth0 user id" };
        ...
        [Fact]
        public async Task PortfolioDoesNotExist_CheckPermission_ReturnNotFound()
        {
            ...
            {
                var response = await client.GetAsync(
                    $"users/{Guid.NewGuid()}/permissions/MangePortfolio/eligibility?portfolioId={nonExistingPortfolioId}"
                );

                response.StatusCode.Should().Be(HttpStatusCode.NotFound);
                var resultJson = await response.Content.ReadAsStringAsync();
                var result = JsonConvert.DeserializeObject<ErrorResponse>(resultJson);
                result.Message.Should().Contain("portfolio");
            }
        }
    }
}
