    ...
    public class GetResetPasswordTokenTests : BaseInMemoryTest
    {
        ...
        public async Task EmailTokenExist_GetResetPasswordToken_ReturnCorrectInformation()
        {
            ...
            using (var client = server.CreateClient(null))
            {
                ...
                var response = await client.GetAsync($"resetPasswordTokens/{resetPasswordToken}");
        ...
        [Fact]
        public async Task EmailTokenDoesNotExist_GetResetPasswordToken_ReturnCorrectInformation()
        {
            using (var server = CreateServerFixture(ServerFixtureConfigurations.InMemoryDb))
            using (var client = server.CreateClient(null))
            {
