        ...
        where TOriginalStartup : class
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            var logger = serviceProvider.GetRequiredService<
                ILogger<MainServiceStartup<TOriginalStartup>>
            >();
            var serverFixtureConfiguration =
                serviceProvider.GetRequiredService<ServerFixtureConfiguration>();
            ...
            foreach (var dependencyService in serverFixtureConfiguration.DependencyServices)
            {
                if (dependencyService.IsMultiRegion)
                {
                    ...
                    {
                        var serviceName = MultiRegionHelper.ServiceName(
                            dependencyService.Name,
                            regionId
                        );
                        ConfigureDependencyService(
                            serviceName,
                            dependencyService,
                            httpHandler,
