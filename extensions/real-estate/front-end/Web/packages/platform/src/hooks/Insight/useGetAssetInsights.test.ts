...
import useGetAssetInsights from './useGetAssetInsights'
...
describe('useGetAssetInsight', () => {
  test('should provide error when exception error happens', async () => {
    jest
      .spyOn(insightsService, 'fetchAssetInsights')
      .mockRejectedValue(new Error('fetch error'))
    const { result } = renderHook(() => useGetAssetInsights(params), {
      wrapper: BaseWrapper,
    })
