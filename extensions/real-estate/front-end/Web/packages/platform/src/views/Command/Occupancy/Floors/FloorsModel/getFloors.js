...
export default function getFloors(floors) {
  ...
  const coordinates = nextFloors.flatMap((floor) => floor.geometry).flat()
  const x = Math.max(...coordinates.map((coordinate) => coordinate[0]))
  const y = Math.max(...coordinates.map((coordinate) => coordinate[1]))

  return nextFloors.map((floor) => ({
    ...floor,
    peoplePosition: Number.isNaN(x) || Number.isNaN(y) ? [] : [x, y],
  }))
}
