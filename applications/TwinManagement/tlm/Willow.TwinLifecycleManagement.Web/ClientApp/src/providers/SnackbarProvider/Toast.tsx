...
export default function Toast({ toast }) {
  ...
  const { snackbarId, isClosing, message, isError, closeButtonLabel, color, isAutoHide = true } = toast;
  ...
  useEffect(() => {
    ...
    // If isAutoHide is false, then the toast will not disappear automatically.
    if (!isAutoHide) {
      return;
    } else {
      start();
...
const ToastContainer = styled.div<{ $isClosing: boolean; $height: string }>`
  ...
  animation: ${({ $isClosing }) => ($isClosing ? exitAnimation : enterAnimation)};

  z-index: ${({ $isClosing }) => ($isClosing ? `calc(var(--z-snackbars) - 1)` : `var(--z-snackbars)`)};
`;
...
const barEnterAnimation = css`
  ${barEnterKeyframe} 5.2s  infinite linear
`;
...
const Text = styled.div<{ $height: string; color?: string }>(({ theme, $height, color }) => ({
  ...theme.font.body.md,
  width: '305px',
  height: $height ?? '64px',
  padding: '21px 24px',
