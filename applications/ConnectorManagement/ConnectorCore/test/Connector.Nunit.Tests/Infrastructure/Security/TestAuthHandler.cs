    ...
    public class TestAuthHandler : AuthenticationHandler<AuthenticationSchemeOptions>
    {
        ...
        protected override Task<AuthenticateResult> HandleAuthenticateAsync()
        {
            if (Context.Request.Headers.TryGetValue("Authorization", out StringValues authorizationValues))
            {
                ...
                {
                    var tokenEncoded = authorizationValues[0].Substring(TestAuthToken.TestScheme.Length + 1);

                    var decodedString = Encoding.UTF8.GetString(Convert.FromBase64String(tokenEncoded));

                    var token = JsonConvert.DeserializeObject<TestAuthToken>(decodedString);
                    var claims = new List<Claim> { new Claim(ClaimTypes.UserData, token.UserId.ToString()) };
