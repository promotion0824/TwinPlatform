        ...
        public void OnException(ExceptionContext context)
        {
            ...
            switch (context.Exception)
            {
                case ApiException apiException:
                    ...
                    logger.LogWarning(apiException, "GlobalExceptionFilter caught a known exception. {errorResponse}", JsonSerializer.Serialize(response));
                    break;

                default:
                    var exception = context.Exception;
                    logger.LogError(exception, "GlobalExceptionFilter caught an exception.");
                    response = new ErrorResponse
                    {
                        StatusCode = (int)HttpStatusCode.InternalServerError,
                        Message = exception.Message,
                        Data = new { ExceptionType = exception.GetType().ToString() },
                        CallStack = exception.ToString().Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries),
                    };
                    break;
            }
