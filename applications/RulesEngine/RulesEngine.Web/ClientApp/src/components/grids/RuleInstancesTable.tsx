...
import { Alert, Box, Button, Checkbox, Grid, Paper, Snackbar, Stack, Typography } from '@mui/material';
import { DataGridPro, DataGridProProps, GridApi, GridColDef, GridColumnVisibilityModel, GridEditSingleSelectCell, GridEditSingleSelectCellProps, GridFilterModel, GridPaginationModel, GridSortDirection, GridSortModel, GridToolbarColumnsButton, GridToolbarContainer, GridToolbarFilterButton, useGridApiContext } from '@mui/x-data-grid-pro';
...
interface IRuleInstancesTableProps {
  ruleId: string,
  pageId: string,
...
const RuleInstancesTable = (query: IRuleInstancesTableProps) => {

  const ruleId = query.ruleId;
  const actions = query.actions;
  const showRuleColumnsByDefault = query.showRuleColumnsByDefault ?? false;

  const filterKey = buildCacheKey(`${query.pageId}_${ruleId}_RuleInstancesTable_FilterModel`);
  const sortKey = buildCacheKey(`${query.pageId}_${ruleId}_RuleInstancesTable_SortModel`);
  const colsKey = buildCacheKey(`${query.pageId}_${ruleId}_RuleInstancesTable_ColumnModel`);
  const paginationKey = buildCacheKey(`${query.pageId}_RuleInstancesTable_PaginationModel`);
    ...
    return (
      ...
      >
        <Paper sx={{ flex: 1, mx: 'auto', width: '90%', p: 1 }}>
          ...
          {actions && <Box p={1}>{actions(ruleInstance)}</Box>}

          <RuleInstanceBindings ruleInstance={ruleInstance} pageId={query.pageId} showInvalidOnly={!isValid} showBindingsToggle={!isValid} />
  ...
  const initialColumns: GridColDef[] =
    [
      ...
      {
        field: 'Disabled', headerName: 'Enabled', width: 100, cellClassName: "MuiDataGrid-cell--textCenter",
        valueOptions: () => { return RuleInstanceBooleanFilter.GetInvertedBooleanFilter(); },
        filterOperators: boolOperators(),
        // just a check box not input editable:true,
...
export default RuleInstancesTable;
