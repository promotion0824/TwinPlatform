...
namespace Yarp.ReverseProxy.Swagger
{
    public sealed class ReverseProxyDocumentFilter : IDocumentFilter
    {
        ...
        )
        {
            ...
            // Deliberately exclude v1 and v2
            if (swaggerDoc.Info.Version == "v3")
            {
                clusters = config.Clusters.Where(p => p.Key != "v1" && p.Key != "v2").ToDictionary(x => x.Key, x => x.Value); // Customized deviation

            }
            else if (config.Swagger.IsCommonDocument)
            {
            ...
            foreach (var clusterKeyValuePair in clusters)
            {
                ...
                foreach (var destination in cluster.Destinations)
                {
                    ...
                    foreach (var swagger in destination.Value.Swaggers)
                    {
                        ...
                        IReadOnlyDictionary<string, (string Match, IEnumerable<string> Methods)> publishedRoutes = null;
                        ...
                        foreach (var swaggerPath in swagger.Paths)
                        {
                            ...
                            foreach (var path in doc.Paths)
                            {
                                ...
                                if (publishedRoutes != null)
                                {
                                    ...
                                    var methods = publishedRoutes[pathKey].Methods;
                                    key = publishedRoutes[pathKey].Match;
        ...
        private static IReadOnlyDictionary<string, (string Match, IEnumerable<string> Methods)> GetPublishedPaths(
            ReverseProxyDocumentFilterConfig config)
        {
            var validRoutes = new Dictionary<string, (string Match, IEnumerable<string> Methods)>(StringComparer.InvariantCultureIgnoreCase); //Critical change !
            ...
            foreach (var route in config.Routes)
            {
                ...
                // Critical change
                var transform = route.Value.Transforms?.Where(t => t.ContainsKey("PathPattern")).SingleOrDefault();

                string pathPattern = null;
                transform?.TryGetValue("PathPattern", out pathPattern);
                string pathKey = pathPattern ?? route.Value.Match.Path;

                if (!validRoutes.ContainsKey(pathKey))
                {

                    validRoutes.TryAdd(pathKey, (route.Value.Match.Path, route.Value.Match.Methods));
                }
                else if (route.Value.Match.Methods != null)
                {
                    validRoutes[pathKey] = (validRoutes[pathKey].Match,
                        validRoutes[pathKey].Methods.Concat(route.Value.Match.Methods));
                }
            }
