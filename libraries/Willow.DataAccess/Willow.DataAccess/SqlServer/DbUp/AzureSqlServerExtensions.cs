...
namespace Willow.DataAccess.SqlServer.DbUp;
...
public static class AzureSqlServerExtensions
{
    ...
    /// </returns>
    public static UpgradeEngineBuilder AzureSqlDatabase(
    ...
    /// A builder for a database upgrader designed for SQL Server databases.
    /// </returns>
    private static UpgradeEngineBuilder AzureSqlDatabase(IConnectionManager connectionManager, string? schema = null)
    {
    ...
    /// <param name="supported">Fluent helper type.</param>
    /// <param name="connectionString">The connection string.</param>
    /// <param name="logger">The <see cref="global::DbUp.Engine.Output.IUpgradeLog"/> used to record actions.</param>
    ...
    public static void AzureSqlDatabase(
        ...
        string? collation = null)
    {
        GetMasterConnectionStringBuilder(connectionString, logger, out string masterConnectionString, out string databaseName);

        using var connection = new SqlConnection(connectionString);
        if (connection.UseManagedIdentity())
        {
            logger?.WriteInformation("Use ManagedIdentity for the connection: {0}", masterConnectionString);
        }

        connection.Open();

        var sqlCommandText = string.Format(CultureInfo.InvariantCulture,
        ...
        string collationString = string.IsNullOrEmpty(collation) ? string.Empty : string.Format(CultureInfo.InvariantCulture, @" COLLATE {0}", collation);
        sqlCommandText = string.Format(CultureInfo.InvariantCulture,
            @"create database [{0}]{1};",
            databaseName,
            collationString);
